...
#ifdef DEBUG_MODE
#include "debug.h"
#define DEBUG_NUM(num) debugNumber(num)
#define DEBUG_WITH_TIME(txt) debugWithTime(txt)
#else
#define DEBUG_NUM(num)
#define DEBUG_WITH_TIME(txt)
#endif
...
volatile BYTE CtrlTrfData[USB_EP0_BUFF_SIZE] NOLOAD;
...
#endif
{
    ...
    {
        //mar 2013/05/02: -> is done in USBWakeFromSuspend USBClearInterruptFlag(USBActivityIFReg,USBActivityIFBitNum);

#if defined(USB_SUPPORT_OTG)
        USBClearInterruptFlag(USBActivityIFReg,USBActivityIFBitNum); //mar 2013/05/02: maybe otg needs this
        U1OTGIR = 0x10;
#else
        USBWakeFromSuspend();
#endif
...
#ifdef AMS_USB_WAKEUP_FROM_SLEEP /* mar 2013/04: do not deactive interrupt here
                                    - we use it also to wakeup from sleep */
#else
    USBActivityIE = 0; 
#endif
